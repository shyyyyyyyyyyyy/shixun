...
//向后台发出ajax请求，并获取响应数据内容。查询所有班级信息
        ...
        success(resp) {//回调函数
            ...
            if (resp.success) {
                ...
                data.forEach(it => {//it即遍历过程中的一个学生对象
                    ...
                    $tr.append("<td>" + it.beginTime + "</td>");
                    $tr.append("<td>" + it.endTime + "</td>");
                    $tr.append("<td>" + it.total + "</td>");
                ...
                let ids = [];
                let map = {};//缓存行信息
                let $trs = $("#tbl>tbody>tr");
                $trs.each(function () {
                    let id = $(this).children().eq(0).children(":checkbox").val();
                    ids.push(id);
                    map["id"] = $(this);
                });

                //查询班级学生总数
                /*let url = "/api/v1/classes/count";
                $.ajax({
                    data: ids
                });*/
            }
